...
resource "aws_security_group_rule" "allow_agents_to_jks_jnlp_port" {
  ...
  protocol          = "tcp"
  type              = "ingress"
  cidr_blocks       = ["${data.aws_network_interface.each_network_interface[count.index].private_ip}/32"]
  description       = "From the NLB to the Jenkins Controller via JNLP and ENI ${data.aws_network_interface.each_network_interface[count.index].id}."
}
...
resource "aws_security_group_rule" "from_private_nlb_network_interfaces" {
  ...
  type              = "ingress"
  cidr_blocks       = ["${data.aws_network_interface.each_network_interface[count.index].private_ip}/32"]
  description       = "From the NLB to the Jenkins Controller via HTTP and ENI ${data.aws_network_interface.each_network_interface[count.index].id}. Required for health check."
}
