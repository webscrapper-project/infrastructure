...
# See jenkins ecs-jenkins plugin documentation: https://plugins.jenkins.io/scalable-amazon-ecs/
...
data "aws_network_interface" "each_network_interface" {
  ...
  filter {
    name   = "subnet-id"
    values = [element([for subnet in var.private_subnets : subnet], count.index)]
  }
}
